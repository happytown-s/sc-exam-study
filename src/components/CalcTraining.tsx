...
const STORAGE_STATS = 'sc-calc-stats'

function loadStats(): CalcStats {
  try {
    const raw = localStorage.getItem(STORAGE_STATS)
    if (raw) return JSON.parse(raw) as CalcStats
  } catch { /* ignore */ }
  return { total: 0, correct: 0, byCategory: {} }
}

function saveStats(s: CalcStats) {
  localStorage.setItem(STORAGE_STATS, JSON.stringify(s))
}

const categories = [...new Set(calcData.map((q) => q.category))]
...
export default function CalcTraining() {
  ...
  const [stats, setStats] = useState<CalcStats>(loadStats)

  useEffect(() => { setStats(loadStats()) }, [mode])

  const startQuiz = useCallback((qList: CalcQ[]) => {
  ...
  const startCategory = (cat: string) => {
    ...
    startQuiz(calcData.filter((q) => q.category === cat))
  }
      ...
      <div>
        <div className="mb-4 p-3 rounded" style={{ background: '#1a1a3e' }}>
          <p className="text-sm" style={{ color: '#ccc' }}>
            Total: {stats.total} | Correct: {stats.correct} | Rate: {stats.total > 0 ? ((stats.correct / stats.total) * 100).toFixed(1) : 0}%
          </p>
        </div>
        <h2 className="text-white font-bold mb-2">Calculation Training</h2>
        <p className="text-sm mb-4" style={{ color: '#888' }}>Practice security calculations with step-by-step solutions</p>
        <div className="grid gap-2">
          {categories.map((cat) => {
            const catQs = calcData.filter((q) => q.category === cat)
  ...
  return (
    <div>
      ...
      <button
        ...
        style={{ background: showCheatsheet ? '#2a2a5e' : '#1a1a3e', color: '#aaa', border: '1px solid #2a2a5e' }}
      >
        {showCheatsheet ? 'Hide' : 'Show'} Cheatsheet
      </button>

      {showCheatsheet && (
        <pre className="mb-4 p-3 rounded text-xs overflow-auto whitespace-pre-wrap" style={{ background: '#1a1a3e', color: '#aaa', maxHeight: '200px' }}>
          {q.cheatsheet}
        </pre>
      )}
